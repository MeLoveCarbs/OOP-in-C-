...
{
    for (int i {0}; i < num; i++) {
        std::cout << "Please enter number [" << i + 1 << "] " << std::endl;
        int num {0};
        std::cin >> num;
        std::shared_ptr<Test> a = std::make_shared<Test> (num);
        vec.push_back(a);
    }
}
void display(const std::vector<std::shared_ptr<Test>>&vec)
{
    int count {1};
    std::cout << "================DATA STORED AS FOLLOWS======================" << std::endl;
    for (const auto &elem: vec) {
        std::cout << "Element " << count << " : [" << elem->get_data() << "]" << std::endl;
        count++;
    }
}
